...
在pre-train的时候, 仅使用VLM backbone进行训练. [[2504.16054v1.pdf#page=5&selection=3,55,6,28|此时所有的action都是用离散的token]], 使用[[2501.09747|FAST]]进行离散化处理, 以获得更高的效率
...
为了增加训练的效果([[2501.09747|FAST]]中有提到), 使用离散的token进行训练. 但是为了inference的效率, 仍然需要continuous action space. 因此使用离散token和连续的token同时进行训练.
...
- [[2501.09747|FAST]]生成的离散token能够看到VLM的全部信息, 但是看不到Action Expert生成的noisy action embedding. 同时, 因为这里是auto-regressive的生成式模型, 因此这个只能看到自己前面的信息, 看不到自己后面的信息.
- Action Expert中, noisy action embedding(用于经过一个action out proj生成[[2210.02747|Flow Matching]]的向量场$v_t$)能看到VLM的全部信息, 但是看不到[[2501.09747|FAST]]生成的离散信息. 并且这个embedding能够完全看到自己. 这个embedding仅仅用于提取信息, 不是生成.
...
- $H(x_{1:M},f_\theta^l(o_t,l))$: cross entropy loss, 是正常LLM训练的loss(包含[[2501.09747|FAST]] encoded action tokens)
...
使用[[2501.09747|FAST]]将连续的动作转换成离散的token
...
类似[[2501.09747|FAST]], 需要将1st and 99th quantile到$[-1,1]$
