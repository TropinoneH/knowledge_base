...
在SSR模式下,渲染到前端的直接就是页面,而不是想Vue的`id="app"`然后用各种类名来封装的,是通过js注入来改变div的样式,这样会导致搜索引擎爬虫找不到相应的资源,会导致引擎的排行下降甚至消失.
...
> 在Vue中引入static或assets中的文件可以通过~/assets/{文件名}或~/static/{文件名}来引入
>
