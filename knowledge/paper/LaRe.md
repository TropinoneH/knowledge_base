...
\caption{LaRe}
	\begin{algorithmic}
		\input:\\1. LLM $\mathcal M$\\2. task information $task$\\3. candidate responses number $n$\\4. pre-collected random state-action pairs $\bar s$\\5. max episodes $\mathcal N^\text{max}$
		\output:\\1. policy network $\pi_\theta$\\2. reward decoder model $f_psi$
		\State 初始化policy network参数$\theta$, reward decoder参数$\psi$, replay buffer $\mathcal B$
		\State 获取candidate response $\xi_1,\cdots,\xi_n\gets\mathcal M(task,role)$
		\State 总结得出improved response $\xi\gets\mathcal M(task,role,\xi_1,\cdots,\xi_n)$
		\State 验证latent reward encode functino $\phi$: $err\gets\textbf{verify}(\phi,\bar s);\xi\gets\mathcal M(task, role,\xi_{1:n},err)$. 相当于是错误反馈
		\For{$episode=1$ \To $\mathcal N^\text{max}$}
			\State 使用当前policy采样一个轨迹$tau$
			\State $\mathcal B\gets\mathcal B\cup\{r\}$
			\State 从replay pool $\mathcal B$中采样一个batch $B=\{\tau_i\}_{i=1}^{|B|}$
			\State 评估latent reward. 使用Loss: $\mathcal L_{\text{RD}}^\phi(\psi)=\mathcal E_{r\sim D}\left[\left(R(\tau)-\sum_{t=1}^Tf_\psi(\phi(s_t,a_t)\right)\right]$
			\State 使用任意有predicted proxy reward $\hat r^{\psi,\phi}=f_\psi(\phi(s,a))$ 的RL算法优化policy
		\EndFor
	\end{algorithmic}
\end{algorithm}
```
