...
- 多路选择器 Multiplexers:
  ...
  <span><img src="./assets/image-20240409082236533.png" style="zoom:30%;" /><img src="./assets/image-20240409082427527.png" style="zoom:25%;" /></span>
...
![./image-20240409082630461.png](./assets/image-20240409082630461.png)
...
- 加法器：
  ...
  ![image-20240409083009400](./assets/image-20240409083009400.png)
...
- 减法器：
  ...
  ![image-20240409083025746](./assets/image-20240409083025746.png)

  如果想要直接在加法器上进行复用（减少电路的复杂度），可以直接使用异或门进行控制是否取反。

  ![image-20240409083144640](./assets/image-20240409083144640.png)
...
- Register寄存器
  ...
  <img src="./assets/image-20240409084503129.png" alt="image-20240409084503129" style="zoom:60%;" />

- PC指针

  直接默认是每次执行PC加4即可
  ![image-20240409085423173](./assets/image-20240409085423173.png)
...
- 内存
  ...
  ![image-20240409091104480](./assets/image-20240409091104480.png)
...
- R-type:
  ...
  ![image-20240409091254619](./assets/image-20240409091254619.png)
...
- I-type
  ...
  ![image-20240409091659197](./assets/image-20240409091659197.png)

  lw等与内存地址交互：

  添加一条通路，与Data Memory进行交互读取。然后使用mux2进行选择（保证原有的I-type和R-type都是正常的）

  ![image-20240409094032792](./assets/image-20240409094032792.png)
...
- S-type
  ...
  ![image-20240409095616652](./assets/image-20240409095616652.png)
...
- Imm Generator
  ...
  ![image-20240409095343759](./assets/image-20240409095343759.png)

## Pipline

single-cycle:

![image-20240418082913648](./assets/image-20240418082913648.png)

pipeline:

![image-20240418082937850](./assets/image-20240418082937850.png)
...
![image-20240418083449148](./assets/image-20240418083449148.png)
...
1. through wait/stall:
   ...
   <img src="./assets/image-20240418092026377.png" alt="image-20240418092026377" style="zoom:33%;" />

2. Forwarding or bypass:

   <img src="./assets/image-20240418092311853.png" alt="image-20240418092311853" style="zoom:33%;" />

   走一个捷径，直接从输出重新回到输入，跳过`WB`

   通过`op1_src`操作数来选择哪一个输入

   ![image-20240418092857705](./assets/image-20240418092857705.png)
...
3. 6. al # of cache lines == capacity of cache/line size
...
<img src="./assets/image-20240430094350954.png" alt="image-20240430094350954" style="zoom:33%;" />
...
<img src="./assets/image-20240507082955587.png" alt="image-20240507082955587" style="zoom:67%;" />

MIMD: multi-thread, multiple instruction operate multiple data

<img src="./assets/image-20240507083008641.png" alt="image-20240507083008641" style="zoom:67%;" />
...
<img src="./assets/image-20240514092552725.png" alt="image-20240514092552725" style="zoom:50%;" />
...
![image-20240514093250782](./assets/image-20240514093250782.png)
...
<img src="./assets/image-20240514090019389.png" alt="image-20240514090019389" style="zoom:30%;" />
...
- Trap: 通过硬件跳转到“trap handler”代码来处理中断或异常的操作
  ...
  <img src="./assets/image-20240516093226680.png" alt="image-20240516093226680" style="zoom:50%;" />
...
![image-20240516094052454](./assets/image-20240516094052454.png)

一条指令最后结束的位置在写入Data Mem的位置，所以异常处理写在Data Mem之前

![image-20240516094323564](./assets/image-20240516094323564.png)
...
![image-20240516095107078](./assets/image-20240516095107078.png)
...
![image-20240521095427700](./assets/image-20240521095427700.png)

根据这个思想设计的Data path:

![image-20240521095522496](./assets/image-20240521095522496.png)
...
> 
> <img src="image-20240521101432120.png" alt="image-20240521101432120" style="zoom:33%;" />

一个程序生成的虚拟内存的地址可以被分成两部分：`page number | offset`

页表可以保存真实的物理地址，这将允许内存空间的不连续使用，增加空间利用率

![image-20240523084057512](./assets/image-20240523084057512.png)
...
![image-20240523082732215](./assets/image-20240523082732215.png)
...
![image-20240523084416218](./assets/image-20240523084416218.png)
...
![image-20240523091135270](./assets/image-20240523091135270.png)
...
![image-20240523091506226](./assets/image-20240523091506226.png)

### Cache Coherence

![image-20240523092311534](./assets/image-20240523092311534.png)

读取没有问题

![image-20240523092508607](./assets/image-20240523092508607.png)
...
![image-20240528083713877](./assets/image-20240528083713877.png)

![image-20240528083729046](./assets/image-20240528083729046.png)
...
![image-20240604082256001](./assets/image-20240604082256001.png)

![image-20240604082318008](./assets/image-20240604082318008.png)

![image-20240604082338344](./assets/image-20240604082338344.png)

![image-20240604082420128](./assets/image-20240604082420128.png)
...
![image-20240604083237728](./assets/image-20240604083237728.png)
...
![image-20240604084744379](./assets/image-20240604084744379.png)
...
![image-20240606092624938](./assets/image-20240606092624938.png)

这个时候其实是无法分辨来源的(e.g. 001和011, 101, 000的距离都是1)

![image-20240606092710661](./assets/image-20240606092710661.png)
...
![image-20240611082022427](./assets/image-20240611082022427.png)

但是储存校验的这一块硬盘很容易出现性能瓶颈。每一块硬盘区域的读写都需要同时对这个P区域读写

raid5：只对修改的内容更改

![image-20240611082314597](./assets/image-20240611082314597.png)
