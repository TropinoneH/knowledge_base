...
tags:
  ...
  - DL
aliases:
  - Introduce to Machine Learning - Fit
---
# Fit
...
如果$X$不是奇异矩阵, 那么$\hat\beta=(X^TX)^{-1}X^Ty$
...
$$E[x]=\int xp(x)dx=\frac1K\sum_{k=1}^Kx_k,x_k\sim p(x)$$
即在$p(x)$中抽样。当$K$足够大，可以认为与期望是相等的。那么有：
$$\min_{\theta}KL(p(y)||p_\theta(y))=C-\frac1N\sum_{i=1}^N\log p_\theta(y_i)$$
去掉常数（因为是对$\theta$求最小），有：
$$\max_\theta l(\theta)=\sum_{i=1}^N\log Pr_\theta(y_i)$$
...
$$\log Pr(G=k|X=x)=-\frac12(x-\hat\mu_k)^T\hat\Sigma^{-1}(x-\hat\mu_k)+\ln\hat\pi_k+C$$
$$=-\frac12||x^*-\hat\mu_k^*||^2+\ln\hat\pi_k+C$$
...
求解：
应用拉格朗日乘数法：
$$\mathcal L(\vec w, b,\vec \alpha)=\frac12||\vec w||^2+\sum\alpha_i(1-y_i(\vec w\cdot\vec x_i-b))$$
...
上面的$\vec x_i^T\cdot\vec x_j$可以使用核函数进行升维
$$\Rightarrow\left\{\begin{matrix}\alpha_i\geq0\\y_if(\vec x_i)-1\geq0\\\alpha_i(y_if(\vec x_i)-1)=0\end{matrix}\right.$$
...
1. 假设每个数据点都是$n$维的数据，即$X=\langle X_1,\cdots,X_n\rangle$，假设$X_i$之间相互独立（高斯分布的朴素贝叶斯假设）
...
3. 根据$N(\mu_i,\Sigma_i)$随机生成数据点$\langle x_1,\cdots,x_n\rangle$

   假设已经知道了$\sigma$，还需要知道$\pi_1,\cdots,\pi_k$和$\mu_{1i},\cdots,\mu_{Ki}$

   观测值：$X=\langle X_1,\cdots,X_n\rangle$，隐变量：$Z$

使用[[Bayes#EM算法|EM算法]]进行估计：

Define $Q(\theta'|\theta)=\mathbb E_{Z|X,\theta}[\log P(X,Z|\theta')],\theta=\langle\pi,\mu_{ji}\rangle$

E-step:
$$P(z(n)=k|x(n),\theta)=\frac{\prod_iN(x_i(n)|z(n)=k,\theta)(\pi^k(1-\pi)^{1-k})}{\sum_{j=0}^1[\prod_iN(x_i(n)|\mu_{ji},\sigma)](\pi^j(1-\pi)^{1-j})}$$
M-step:
$$\pi\leftarrow\frac{1}{N}\sum_{n=1}^N\mathbb E[z(n)]$$
$$\mu_{ji}\leftarrow\frac{\sum_{n=1}^NP(z(n)=j|x(n),\theta)x_i(n)}{\sum_{n=1}^NP(z(n)=j|x(n),\theta)}$$
...
$$\mathop{\arg\min}_{h_1,h_2}\sum_{l=1}^2\sum_{i=1}^{m_l}l(h_l(x_i),y_i)+C\sum_{i=1}^{m_u}\mathbf{agreement}(h_1(x_i),h_2(x_i))$$
...
相似度可以使用Gaussian [[#kernel函数|Kernel]]来计算：$K(x,z)=\exp\left[-\frac{||x_i-x_j||^2}{2\sigma^2}\right]$，其中$x_i$和$x_j$是两个样本点的特征向量
...
令$f=\beta X$,
$$\Rightarrow f^TLf=\beta^TX^TLX\beta\Rightarrow \min\beta^TX^TLX\beta+\lambda||y-X\beta||^2_2+\alpha||\beta||^2$$
