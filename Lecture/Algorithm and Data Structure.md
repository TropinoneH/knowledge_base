...
$O(n)$: $\exists k,\ s.t.\ \text{when }x>k,f(x)\leq c\cdot g(x))$. 那么就有时间复杂度为$O(g(n))$

$\Theta(n)$: $\exists k,\ s.t.\text{ when }x>k,\ c_1g(x)<f(x)<c_2g(x)$, 有$\Theta(g(n))$
...
![[Pasted image 20231020084923.png]]
