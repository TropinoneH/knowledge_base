...
- 多路选择器 Multiplexers:
  ...
  ![[Pasted image 20240409082236.png|200]]![[Pasted image 20240409082427.png|244]]
...
![./Pasted image 20240409082630.png](Pasted image 20240409082630.png)
...
- 加法器：
  ...
  ![[Pasted image 20240409083009.png]]
...
- 减法器：
  ...
  ![[Pasted image 20240409083025.png]]

  如果想要直接在加法器上进行复用（减少电路的复杂度），可以直接使用异或门进行控制是否取反。

  ![[Pasted image 20240409083144.png]]
...
- Register寄存器
  ...
  ![[Pasted image 20240409084503.png]] 

- PC指针

  直接默认是每次执行PC加4即可
  ![[Pasted image 20240409085423.png]]
...
- 内存
  ...
  ![[Pasted image 20240409091104.png]]
...
- R-type:
  ...
  ![[Pasted image 20240409091254.png]]
...
- I-type
  ...
  ![[Pasted image 20240409091659.png]]

  lw等与内存地址交互：

  添加一条通路，与Data Memory进行交互读取。然后使用mux2进行选择（保证原有的I-type和R-type都是正常的）

  ![[Pasted image 20240409094032.png]]
...
- S-type
  ...
  ![[Pasted image 20240409095616.png]]
...
- Imm Generator
  ...
  ![[Pasted image 20240409095343.png]]

## Pipline

single-cycle:

![[Pasted image 20240418082913.png]]

pipeline:

![[Pasted image 20240418082937.png]]
...
![[Pasted image 20240418083449.png]]
...
1. through wait/stall:
   ...
   ![[Pasted image 20240418092026.png]]

2. Forwarding or bypass:

   ![[Pasted image 20240418092311.png]]

   走一个捷径，直接从输出重新回到输入，跳过`WB`

   通过`op1_src`操作数来选择哪一个输入

   ![[Pasted image 20240418092857.png]]
...
2. mapping methods:
   3. DIrect mapping: 直接映射，只有一个cache set，只需要选择tag
   4. Fully associativity: 每一个cache line都是一个cache set，不区分tag
   5. N-ways Associative: 分了N个cache set
6. 6. al # of cache lines == capacity of cache/line size
...
![[Pasted image 20240507082955.png]]

MIMD: multi-thread, multiple instruction operate multiple data

![[Pasted image 20240507083008.png]]
...
![[Pasted image 20240514092552.png]]
...
![[Pasted image 20240514093250.png]]
...
![[Pasted image 20240514090019.png|200]]
...
> $$
> \text{mouse polling}=30[polls/s] * 400[cc/polls]=12000[cc/s]\\
> \text{\%processor of polling}=\frac{12000[cc/s]}{10^9[cc/s]}=0.0012\%
> $$
> 可知：轮询鼠标对处理器影响很小
...
- Trap: 通过硬件跳转到“trap handler”代码来处理中断或异常的操作
  ...
  ![[Pasted image 20240516093226.png|300]]
...
![[Pasted image 20240516094052.png]]

一条指令最后结束的位置在写入Data Mem的位置，所以异常处理写在Data Mem之前

![[Pasted image 20240516094323.png]]
...
![[Pasted image 20240516095107.png]]
...
![[Pasted image 20240521095427.png]]

根据这个思想设计的Data path:

![[Pasted image 20240521095522.png]]
...
> [!info]
> 常见的混淆点：字节、字、块、页都是查看内存的不同方式
> 
> ![[Pasted image 20240521101432.png]]

一个程序生成的虚拟内存的地址可以被分成两部分：`page number | offset`

页表可以保存真实的物理地址，这将允许内存空间的不连续使用，增加空间利用率

![[Pasted image 20240523084057.png]]
...
![[Pasted image 20240523082732.png]]
...
![[Pasted image 20240523084416.png]]
...
![[Pasted image 20240523091135.png]]
...
![[Pasted image 20240523091506.png]]
...
![[Pasted image 20240523092508.png]]
...
![[Pasted image 20240528083713.png]]

![[Pasted image 20240528083729.png]]
...
![[Pasted image 20240604082256.png]]

![[Pasted image 20240604082318.png]]

![[Pasted image 20240604082338.png]]

![[Pasted image 20240604082420.png]]
...
![[Pasted image 20240604083237.png]]
...
![[Pasted image 20240604084744.png]]
...
![[Pasted image 20240606092624.png]]

这个时候其实是无法分辨来源的(e.g. 001和011, 101, 000的距离都是1)

![[Pasted image 20240606092710.png]]
...
![[Pasted image 20240611082022.png]]

但是储存校验的这一块硬盘很容易出现性能瓶颈。每一块硬盘区域的读写都需要同时对这个P区域读写

raid5：只对修改的内容更改

![[Pasted image 20240611082314.png]]
