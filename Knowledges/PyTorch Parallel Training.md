...
启动指令:
```shell
torchrun --standalone --nnodes 1 --nproc-per-node 8 <path/to/run> <--args>
```
- `--standalone`: 是单节点训练, 不需要创建master线程进行多节点通信
- `--nnodes`: 有多少个节点(主机). 如果是多主机共同训练, 需要设置host和port的通信
- `--nproc-per-node`: 每个节点开多少训练进程. 应该和用到多少卡相匹配
- `<path/to/run> <--args>`: 正常通过`python <path/to/run> <--args>`启动单线程的指令
